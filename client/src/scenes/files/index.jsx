...
const Files = () => {
  ...
  const fetchFiles = useCallback(async () => {
    setLoading(true);
    try {
      // La logique pour récupérer les fichiers sera ajoutée ici.
      // Pour l'instant, nous utilisons des données statiques.
      setFiles([
        // Exemples de fichiers. Remplacez par les données de votre API.
        { id: 1, name: "document1.pdf", status: "processed" },
        { id: 2, name: "image.png", status: "pending" },
      ]);
    } catch (error) {
  ...
  const handleUpload = async (event) => {
    ...
    try {
      ...
      toast.success(response.data.message || "Fichiers uploadés avec succès!");
      fetchFiles();
    } catch (error) {
      console.error("Erreur lors de l'upload:", error);
      toast.error(
        error.response?.data?.message || "Erreur lors de l'upload des fichiers."
      );
    }
  };
  ...
  const handleProcess = async (options) => {
    ...
    try {
      const response = await axios.post("/api/data/process", {
        file_id: selectedFile.id,
        ...options,
      });
      toast.success(response.data.message || "Traitement démarré!");
      handleCloseModal();
      fetchFiles();
    } catch (error) {
  ...
  const handleDelete = async (fileId) => {
    ...
    toast.info(`Logique de suppression pour le fichier ${fileId} à implémenter.`);
  };
  ...
  const columns = [
    ...
    { field: "name", headerName: "Nom du fichier", flex: 2 },
    {
      field: "status",
      headerName: "Statut",
      flex: 1,
  ...
  return (
    <Box m="1.5rem 2.5rem">
      <ToastContainer />
      <Header
        title={`FICHIERS DU PROJET ${projectId}`}
        subtitle="Gérez les fichiers de votre projet"
      />
