...
const Projects = () => {
  ...
  const confirmDelete = async () => {
    try {
      // Mettez ici la logique pour supprimer le projet.
      // Par exemple: await axios.delete(`/api/data/projects/${deleteConfirmation.projectId}`);
      toast.success("Projet supprimé avec succès!");
      setProjects(
        projects.filter((p) => p.id !== deleteConfirmation.projectId)
      );
    } catch (error) {
      ...
      renderCell: (params) => (
        <Box>
          <Button
            variant="contained"
            color="secondary"
            onClick={() => navigate(`/files/${params.row.id}`)}
            sx={{ marginRight: "10px" }}
          >
            Gérer les fichiers
          </Button>
  ...
  return (
    <Box m="1.5rem 2.5rem">
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setDeleteConfirmation({ isOpen: false, projectId: null })}>
            Annuler
          </Button>
